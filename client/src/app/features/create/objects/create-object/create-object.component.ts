  ...
  implements OnInit, AfterViewInit, DoCheck, OnDestroy
{
  ...
  @ViewChild("availableObjectsComponent")
  availableObjectsComponent: AvailableObjectsComponent;
  ...
  submit(): void {
    if (this.objForm.controls != undefined) {
      for (const key in this.objForm.controls) {
        if (
          this.objForm.get(key).value !== null &&
          this.objForm.get(key).value !== ""
        ) {
          if (typeof this.objForm.get(key).value != "string") {
            ...
            this.newObject.push({
              key: key,
              value: this.objForm.get(key).value,
            });
          }
        }
      }
      ...
      const observer = {
        ...
        complete: () => {
          this._confirmationDialogService.openDialog(
            `Deseja criar um novo objeto?`,
            () => this.resetCreation(),
            () => this.doNotResetCreation()
          );
        },
      };

      if (this.checkMandatoryObject() && this.checkMandatoryMetadata()) {
        var obj = new Objects(this.objectType, this.newObject, this.metadata);
      ...
      this._notificationsService.info(
        "Atenção!",
        "Ao menos um campo do objeto precisa ser preenchido.",
        5000
      );
    }
  }
  ...
  checkMandatoryObject(): boolean {
    if (this.newObject.length < 1) {
      this._notificationsService.error(
        "Erro!",
        "O objeto deve conter ao menos um campo preenchido.",
        5000
      );
      return false;
    }
  ...
  checkMandatoryMetadata(): boolean {
    if (this.metadata.length < 1) {
      this._notificationsService.error(
        "Erro!",
        "O objeto deve ter metadados associados.",
        5000
      );
      return false;
    }
